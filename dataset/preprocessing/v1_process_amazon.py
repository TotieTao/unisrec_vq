...
def preprocess_rating(args):
    ...
    rating_inters_h = filter_inters(rating_inters_h, rating_users, can_items=meta_items,
                                  user_k_core_threshold=args.user_k,
                                  item_k_core_threshold=args.item_k)
    print('The number of raw inters_s: ', len(rating_inters_s))
    rating_inters_s = filter_inters(rating_inters_s, rating_users, can_items=meta_items,
                                  user_k_core_threshold=2,
                                  item_k_core_threshold=args.item_k)
    print('The number of raw inters_l: ', len(rating_inters_l))
    rating_inters_l = filter_inters(rating_inters_l, rating_users, can_items=meta_items,
                                  user_k_core_threshold=args.user_k,
                                  item_k_core_threshold=args.item_k)


    # sort interactions chronologically for each user
    rating_inters_h = make_inters_in_order(rating_inters_h)
    rating_inters_s = make_inters_in_order(rating_inters_s)
    rating_inters_l = make_inters_in_order(rating_inters_l)
    print('\n')


    # return: list of (user_ID, item_ID, rating, timestamp)
    return rating_inters_h, rating_inters_s, rating_inters_l
...
def generate_item_embedding(args, datatype, item_text_list, item2index, tokenizer, model, word_drop_ratio=-1):
    ...
    file = os.path.join(args.output_path, args.dataset,
                        args.dataset + datatype + '.feat' + suffix + args.emb_type)
...
def parse_args():
    ...
    parser.add_argument('--user_k', type=int, default=2, help='user k-core filtering')
    parser.add_argument('--item_k', type=int, default=2, help='item k-core filtering')
...
if __name__ == '__main__':
    ...
    rating_inters_h, rating_inters_s, rating_inters_l = preprocess_rating(args)

    # load item text from raw meta data file
    item_text_list_h = preprocess_text(args, rating_inters_h)
    item_text_list_s = preprocess_text(args, rating_inters_s)
    item_text_list_l = preprocess_text(args, rating_inters_l)


    # split train/valid/test
    # 后面挑17年历史数据做推荐任务，1415年数据做偏好预测任务
    train_inters_h, valid_inters_h, test_inters_h, user2index_h, item2index_h = generate_training_data(args, rating_inters_h)
    train_inters_s, valid_inters_s, test_inters_s, user2index_s, item2index_s = generate_training_data(args, rating_inters_s)
    train_inters_l, valid_inters_l, test_inters_l, user2index_l, item2index_l = generate_training_data(args, rating_inters_l)
    ...
    generate_item_embedding(args, ".h", item_text_list_h, item2index_h, plm_tokenizer, plm_model, word_drop_ratio=-1)
    generate_item_embedding(args, ".s", item_text_list_s, item2index_s, plm_tokenizer, plm_model, word_drop_ratio=-1)
    generate_item_embedding(args, ".l", item_text_list_l, item2index_l, plm_tokenizer, plm_model, word_drop_ratio=-1)

    # pre-stored word drop PLM embs
    if args.word_drop_ratio > 0:
        generate_item_embedding(args, ".h", item_text_list_h, item2index_h, plm_tokenizer, plm_model, word_drop_ratio=args.word_drop_ratio)
        generate_item_embedding(args, ".s", item_text_list_s, item2index_s, plm_tokenizer, plm_model, word_drop_ratio=args.word_drop_ratio)
        generate_item_embedding(args, ".l", item_text_list_l, item2index_l, plm_tokenizer, plm_model, word_drop_ratio=args.word_drop_ratio)

    # save interaction sequences into atomic files
    convert_to_atomic_files(args, ".h", train_inters_h, valid_inters_h, test_inters_h)
    convert_to_atomic_files(args, ".s", train_inters_s, valid_inters_s, test_inters_s)
    convert_to_atomic_files(args, ".l", train_inters_l, valid_inters_l, test_inters_l)

    # save useful data
    # write_text_file(item_text_list_h, os.path.join(args.output_path, args.dataset, f'{args.dataset}.h.text'))
    write_remap_index(user2index_h, os.path.join(args.output_path, args.dataset, f'{args.dataset}.h.user2index'))
    write_remap_index(item2index_h, os.path.join(args.output_path, args.dataset, f'{args.dataset}.h.item2index'))

    # write_text_file(item_text_list_s, os.path.join(args.output_path, args.dataset, f'{args.dataset}.s.text'))
    write_remap_index(user2index_s, os.path.join(args.output_path, args.dataset, f'{args.dataset}.s.user2index'))
    write_remap_index(item2index_s, os.path.join(args.output_path, args.dataset, f'{args.dataset}.s.item2index'))

    # write_text_file(item_text_list_l, os.path.join(args.output_path, args.dataset, f'{args.dataset}.l.text'))
    write_remap_index(user2index_l, os.path.join(args.output_path, args.dataset, f'{args.dataset}.l.user2index'))
    write_remap_index(item2index_l, os.path.join(args.output_path, args.dataset, f'{args.dataset}.l.item2index'))

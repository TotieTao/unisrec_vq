...
def tsne_plot(objects: np.ndarray, prototypes: np.ndarray, object_legend_text: str = 'Object', perplexity: int = 10,
    ...
    """
    # tsne = TSNE(perplexity=perplexity, metric='cosine', init='pca', learning_rate=200.0, random_state=42)
    ...
    plt.scatter(tsne_protos[:, 0], tsne_protos[:, 1], s=15, c='#d73027', alpha=0.9, label='Prototypes')
...
def tsne_plot_coarse(objects: np.ndarray, prototypes: np.ndarray, coarse_prototypes: np.ndarray, object_legend_text: str = 'Object', perplexity: int = 10,
              path_save_fig: str = None):
    """
    Creates a TSNE plot to visualize the object embeddings and the prototypes in the same space.
    :param objects: Object (users/items) embedding to plot in the same space of the prototypes
    ...
    # tsne = TSNE(perplexity=perplexity, metric='cosine', init='pca', learning_rate=200.0, random_state=42)
    tsne = TSNE(perplexity=perplexity, metric='cosine', init='pca', learning_rate='auto', random_state=42)
    tsne_results = tsne.fit_transform(np.vstack([coarse_prototypes,prototypes, objects]))
    tsne_protos_coarse = tsne_results[:len(coarse_prototypes)]
    tsne_protos = tsne_results[len(coarse_prototypes):len(prototypes)+len(coarse_prototypes)]
    tsne_embeds = tsne_results[len(coarse_prototypes)+len(prototypes):]

    plt.figure(figsize=(6, 6), dpi=100)
    plt.scatter(tsne_embeds[:, 0], tsne_embeds[:, 1], s=10, alpha=0.6, c='#74add1', label=object_legend_text)
    plt.scatter(tsne_protos[:, 0], tsne_protos[:, 1], s=14, c='#d73027', alpha=0.8, label='Prototypes')
    plt.scatter(tsne_protos_coarse[:, 0], tsne_protos_coarse[:, 1], s=15, c='#880000', alpha=0.9, label='Coarse Prototypes')
